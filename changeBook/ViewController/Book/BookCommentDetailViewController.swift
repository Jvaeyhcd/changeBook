//
//  BookCommentDetailViewController.swift
//  changeBook
//
...
class BookCommentDetailViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func praiseComment() {
        ...
        self.viewModel.likeBookComment(commentId: self.comment.id, success: { [weak self] (data) in
    ...
    private func getCommentDetail(page: Int) {
        ...
        self.viewModel.getBookCommentDetail(bookCommentId: self.comment.id, page: page, cache: { [weak self] (data) in
