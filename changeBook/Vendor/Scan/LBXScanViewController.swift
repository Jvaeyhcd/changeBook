...
open class LBXScanViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override open func viewDidLoad() {
        ...
        self.videoPreView = UIView.init(frame: CGRect(x: 0, y: 0, width: kScreenWidth, height: kScreenHeight - kNavHeight))
    ...
    open func setNeedPlaySound(needSound: Bool) {
        isNeedPlaySound = needSound
    }
    ...
    open func startScan()
    {
        ...
        if (scanObj == nil)
        {
            ...
            {
                cropRect = LBXScanView.getScanRectWithPreView(preView: self.videoPreView!, style:scanStyle! )
            }
            ...
            scanObj = LBXScanWrapper(videoPreView: self.videoPreView!, objType:arrayCodeType!, isCaptureImg: isNeedCodeImage,cropRect:cropRect, success: { [weak self] (arrayResult) -> Void in
    ...
    open func drawScanView()
    {
        if qRScanView == nil
        {
            qRScanView = LBXScanView(frame: CGRect(x: 0,
                                                   y: 0,
                                                   width: kScreenWidth,
                                                   height: kScreenHeight - kNavHeight),vstyle:scanStyle! )
    ...
    open func handleCodeResult(arrayResult:[LBXScanResult])
    {
        ...
        {
            BLog(log: "\(String(describing: result.strScanned))")
        }
